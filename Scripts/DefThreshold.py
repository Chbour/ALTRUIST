...
class DefineThreshold(object):
    ...
    def __init__(self, dataframe, model_name, path_dict_concept_keywords="data/dict_concepts_keywords.txt"):
        self.model = SentenceTransformer(model_name)
        self.df = dataframe
        self.dict_concept_keywords = json.load(open(path_dict_concept_keywords))

    def ApplySimilarities(self):
        df = self.df
        model = self.model
