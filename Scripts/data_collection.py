...
class InitialisationTwitter(object):
    ...
    def __init__(self, path_to_keywords="data/keywords.txt", path_to_data="data/connection_data.txt"):
        self.keywords = self.keywords_to_list(path_to_keywords)
        self.connec_data = self.connection_data(path_to_data)

    def keywords_to_list(self, path_to_keywords):
        """
        It takes a file path as an argument, opens the file, reads the file, splits the file into a list,
        and returns the list
        
        :param path_to_keywords: The path to the file containing the keywords
        :return: A list of keywords
        """
        my_file = open(path_to_keywords, "r")
        data = my_file.read()
        keywords = data.split("\n")
        return keywords
    ...
    def connection_to_twitter(self):
        ...
        """
        CONSUMER_KEY = self.connec_data["twitter_consumer_key"]
        CONSUMER_SECRET = self.connec_data["twitter_consumer_secret"]
        ACCESS_TOKEN = self.connec_data["twitter_access_token"]
        ACCESS_TOKEN_SECRET = self.connec_data["twitter_token_secret"]
